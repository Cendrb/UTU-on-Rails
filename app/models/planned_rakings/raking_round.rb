class RakingRound < ActiveRecord::Base
  ...
  def already_rekt_count
    return ActiveRecord::Base.connection.execute("SELECT COUNT(*) FROM(SELECT DISTINCT ON (planned_raking_entries.class_member_id) * FROM planned_raking_entries WHERE(planned_raking_entries.raking_round_id = #{id} AND planned_raking_entries.finished = true)) AS penis").values[0][0].to_i
  end

  def already_signed_up_count
  ...
  def not_signed_up_yet_count
    return planned_raking_list.sclass.class_members.count - already_signed_up_count
  end
